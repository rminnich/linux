...
{
	writel(reg, ioapic_base);
	return readl(ioapic_base + 0x10);
}

void io_apic_write(void *ioapic_base, u32 reg, u32 value)
{
	writel(reg, ioapic_base);
	writel(reg, ioapic_base + 0x10);
}
...
void clear_ioapic(void *ioapic_base)
{
	...
	printk("IOAPIC: Clearing IOAPIC at %p\n", ioapic_base);
	...
	for (i = 0; i < ioapic_interrupts; i++) {
		...
		printk("IOAPIC: reg 0x%08x value 0x%08x 0x%08x\n",
		       i, high, low);
	}

	if (io_apic_read(ioapic_base, 0x10) == 0xffffffff) {
		printk("IOAPIC not responding.\n");
		return;
	}
}
...
void set_ioapic_id(void *ioapic_base, u8 ioapic_id)
{
	...
	printk("IOAPIC: Initializing IOAPIC at 0x%p\n",
	       ioapic_base);
	printk("IOAPIC: Bootstrap Processor Local APIC = 0x%02x\n",
	       bsp_lapicid);

	if (ioapic_id) {
		printk("IOAPIC: ID = 0x%02x\n", ioapic_id);
	...
	printk("IOAPIC: Dumping registers\n");
	for (i = 0; i < 3; i++)
...
/* I think the kernel does this step.*/
#if 0
