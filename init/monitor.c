...
int mon_kerninfo(int argc, char **argv, struct hw_trapframe *hw_tf)
{
	...
	early_printk("Special kernel symbols:\n");
	early_printk("  _start %016x (virt)  %016x (phys)\n", _start, (uintptr_t)(_start - KERNBASE));
	early_printk("  etext  %016x (virt)  %016x (phys)\n", etext, (uintptr_t)(etext - KERNBASE));
	early_printk("  end    %016x (virt)  %016x (phys)\n", end, (uintptr_t)(end - KERNBASE));
	early_printk("Kernel executable memory footprint: %dKB\n",
...
static int __backtrace(int argc, char **argv, struct hw_trapframe *hw_tf)
{
	...
	if (argc != 3) {
		early_printk("Need either no arguments, or two (PC and FP) in hex\n");
		return 1;
	}
	pc = strtol(argv[1], 0, 16);
	fp = strtol(argv[2], 0, 16);
	early_printk("Backtrace from instruction %p, with frame pointer %p\n", pc, fp);
...
{
	early_printk("[Scottish Accent]: She's goin' down, Cap'n!\n");
	//reboot();
...
static int __showmapping(int argc, char **argv, struct hw_trapframe *hw_tf)
{
	...
	pid_t pid;
	if (argc < 3) {
		early_printk("Shows virtual -> physical mappings for a virt addr range.\n");
		early_printk("Usage: showmapping PID START_ADDR [END_ADDR]\n");
		early_printk("    PID == 0 for the boot pgdir\n");
		return 1;
	}
	...
	if (!pid) {
		...
		p = pid2proc(pid);
		if (!p) {
			early_printk("No proc with pid %d\n", pid);
			return 1;
		}
		pgdir = p->env_pgdir;
	}
	...
	if (size/PGSIZE > 512) {
		early_printk("Not going to do this for more than 512 items\n");
		return 1;
	}
...
static void print_info_handler(struct hw_trapframe *hw_tf, void *data)
{
	...
	early_printk("----------------------------\n");
	early_printk("This is Core %d\n", core_id());
	early_printk("Timestamp = %lld\n", tsc);
#ifdef CONFIG_X86
	early_printk("Hardware core %d\n", hw_core_id());
	early_printk("MTRR_DEF_TYPE = 0x%08x\n", read_msr(IA32_MTRR_DEF_TYPE));
	early_printk("MTRR Phys0 Base = 0x%016llx, Mask = 0x%016llx\n",
	        read_msr(0x200), read_msr(0x201));
	early_printk("MTRR Phys1 Base = 0x%016llx, Mask = 0x%016llx\n",
	        read_msr(0x202), read_msr(0x203));
	early_printk("MTRR Phys2 Base = 0x%016llx, Mask = 0x%016llx\n",
	        read_msr(0x204), read_msr(0x205));
	early_printk("MTRR Phys3 Base = 0x%016llx, Mask = 0x%016llx\n",
	        read_msr(0x206), read_msr(0x207));
	early_printk("MTRR Phys4 Base = 0x%016llx, Mask = 0x%016llx\n",
	        read_msr(0x208), read_msr(0x209));
	early_printk("MTRR Phys5 Base = 0x%016llx, Mask = 0x%016llx\n",
	        read_msr(0x20a), read_msr(0x20b));
	early_printk("MTRR Phys6 Base = 0x%016llx, Mask = 0x%016llx\n",
	        read_msr(0x20c), read_msr(0x20d));
	early_printk("MTRR Phys7 Base = 0x%016llx, Mask = 0x%016llx\n",
...
{
#if 0
	early_printk("Number of Cores detected: %d\n", num_cores);
	early_printk("Calling CPU's ID: 0x%08x\n", core_id());
...
int mon_bin_ls(int argc, char **argv, struct hw_trapframe *hw_tf)
{
	...
	if (!bin_dir) {
		early_printk("No /bin directory!\n");
		return 1;
	}
	early_printk("Files in /bin:\n-------------------------------\n");
	do {
...
int mon_bin_run(int argc, char **argv, struct hw_trapframe *hw_tf)
{
#if 0
	if (argc < 2) {
		early_printk("Usage: bin_run FILENAME\n");
		return 1;
	}
	struct file *program;
	int retval = 0;
	...
	if (strlcat(buf, argv[1], sizeof(buf)) > sizeof(buf)) {
		early_printk("Filename '%s' too long!\n", argv[1]);
		return 1;
	}
	program = do_file_open(buf, O_READ, 0);
	if (!program) {
		early_printk("No such program!\n");
		return 1;
	}
...
int mon_procinfo(int argc, char **argv, struct hw_trapframe *hw_tf)
{
	...
	if (argc < 2) {
		early_printk("Usage: procinfo OPTION\n");
		early_printk("\tall: show all active pids\n");
		early_printk("\tpid NUM: show a lot of info for proc NUM\n");
		early_printk("\tunlock: unlock the lock for the ADDR (OMG!!!)\n");
		early_printk("\tkill NUM: destroy proc NUM\n");
		return 1;
	}
	if (!strcmp(argv[1], "all")) {
		...
		if (argc < 3) {
			early_printk("Give me a pid number.\n");
			return 1;
		}
		if (argc >= 4)
		...
		if (argc != 3) {
			early_printk("Gimme lock address!  Me want lock address!.\n");
			return 1;
		}
		spinlock_t *lock = (spinlock_t*)strtol(argv[2], 0, 16);
		if (!lock) {
			early_printk("Null address...\n");
			return 1;
		}
		spin_unlock(lock);
		...
		if (!p) {
			early_printk("No such proc\n");
			return 1;
		}
		proc_destroy(p);
		proc_decref(p);
	} else {
		early_printk("Bad option\n");
		return 1;
	}
#endif
	return 0;
...
int mon_kfunc(int argc, char **argv, struct hw_trapframe *hw_tf)
{
	...
	if (argc < 2) {
		early_printk("Usage: kfunc FUNCTION [arg1] [arg2] [etc]\n");
		early_printk("Use 0x with hex arguments.  Can take 6 args.\n");
		return 1;
	}
	func = (void*)get_symbol_addr(argv[1]);
	if (!func) {
		early_printk("Function not found.\n");
		return 1;
	}
	...
	case 8:
		...
		break;
	default:
		early_printk("Bad number of arguments.\n");
		return -1;
	}
	early_printk("%s (might have) returned %p\n", argv[1], ret);
#endif
	return 0;
}
...
static int runcmd(char *real_buf, struct hw_trapframe *hw_tf) {
	...
	while (1) {
		...
		if (argc == MAXARGS-1) {
			early_printk("Too many arguments (max %d)\n", MAXARGS);
			return 0;
		}
...
void monitor(void)
{
	...
	early_printk("Type 'help' for a list of commands.\n");
...
static void show_msr(struct hw_trapframe *unused, void *v)
{
	...
	val = read_msr(msr);
	early_printk("%d: %08x: %016llx\n", core, msr, val);

	early_printk("complete me\n");
	return;
...
int mon_msr(int argc, char **argv, struct hw_trapframe *hw_tf)
{
	...
	uint32_t msr;
	if (argc < 2 || argc > 3) {
		early_printk("Usage: msr register [value]\n");
		return 1;
	}
